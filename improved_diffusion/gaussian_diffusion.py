...
from .conditioner import statistical_align
...
class GaussianDiffusion:
    ...
    def two_stage_diffusion_step_sampling(self, model, shape, img, clip_denoised=True, denoised_fn=None,
                                          model_kwargs=None, device=None, progress=True,
                                          ddim_stage_ratio=500, eta=0.0, ddim_step_size=2):
        ...
        # -------------------------------------------------------------------------------------
        # 生成跨步后的时间步索引
        x0_stage_start = ddim_stage_ratio
        x0_indices = []
        current = self.num_timesteps - 1
        while current >= x0_stage_start:
            x0_indices.append(current)
            current -= ddim_step_size
        eps_indices = list(range(x0_stage_start - 1, -1, -1))  # eps阶段按原步长
        indices = x0_indices + eps_indices  # 合并时间步
        # -------------------------------------------------------------------------------------
        ...
        for k in tqdm(range(len(indices))):
            ...
            with th.no_grad():
                ...
                model_output = model(img, self._scale_timesteps(t), **model_kwargs)
                pred_eps = model_output['pred_eps']

                if i >= ddim_stage_ratio:  # x0预测阶段，使用跨步
                    # 计算跨步后的alpha_bar_prev
                    if k < len(indices) - 1:
                        next_i = indices[k + 1]
                    else:
                        next_i = 0  # 处理最后一个时间步
                    next_i_tensor = th.tensor([next_i] * shape[0], device=device)
                    x = img[:, -1:, ...]
                    alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
                    alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod, next_i_tensor, x.shape)

                    out = self.p_mean_variance_simple(
                        img,
                        t,
                        clip_denoised=clip_denoised,
                        denoised_fn=denoised_fn,
    ...
    def ddim_sample(
            self, model, x, t, pack, clip_denoised=True, denoised_fn=None, model_kwargs=None, eta=0.0,
    ):
        """
        ...
        eps = self._predict_eps_from_xstart(x, t, out["pred_xstart"])  # 预测的x0
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, x.shape)
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, x.shape)  # α^_(t-1)
        # eta=0时sigma=0
        sigma = (
                eta
                * th.sqrt((1 - alpha_bar_prev) / (1 - alpha_bar))
    ...
    def training_losses(self, model, x_start, t, cur_step,
                        t2_gamma, t2_fft, map_myo_motion, map_myo_color,
                        cine_LH, cine_LH_minus, cine_LH_plus, label,
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start[:, -1:, ...])  # 加的噪声
        ...
        noise_lge = self.q_sample(lge, t, noise=noise)  # 扰动图xt
        cine_LH_all = torch.cat([cine_LH_minus, cine_LH, cine_LH_plus], dim=1)  # (b, 3, 25, 128, 128)

        cine = replace_with_noise(cine.float())
        map_myo_motion = replace_with_noise(map_myo_motion.float())
        map_myo_color = replace_with_noise(map_myo_color.float())
        t2 = replace_with_noise(t2.float())
        t2_gamma = replace_with_noise(t2_gamma.float())
        t2_fft = replace_with_noise(t2_fft.float())

        model_x_t = th.cat([cine.float(), map_myo_motion.float(), map_myo_color.float(),
                            t2.float(), t2_gamma.float(), t2_fft.float(), noise_lge.float()], dim=1)
        model_kwargs = {'cine': cine_LH_all.float(), 'y': label}

        x_start = lge
        x_t = noise_lge
        # ------------------------------------------------------------------------------
        def compute_loss(x_start, x_t, model_output, noise, prefix, pred_type):
            terms = {}
            if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
                terms[f"loss_{prefix}"] = self._vb_terms_bpd(
                    model=model,
                    x_start=x_start,
                    x_t=x_t,
                    t=t,
                ...
                if self.loss_type == LossType.RESCALED_KL:
                    terms[f"loss_{prefix}"] *= self.num_timesteps
                ...
                if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
                    ...
                    terms[f"vb_{prefix}"] = self._vb_terms_bpd(
                    ...
                    if self.loss_type == LossType.RESCALED_MSE:
                        terms[f"vb_{prefix}"] *= self.num_timesteps / 1000.0
                ...
                terms[f"mse_{prefix}"] = mean_flat((target - model_output) ** 2)
                terms[f"mae_{prefix}"] = mean_flat(th.abs(target - model_output))
            else:
                raise NotImplementedError(self.loss_type)
            return terms

        output = model(model_x_t, self._scale_timesteps(t), **model_kwargs)
        terms_eps = compute_loss(x_start, x_t, output['pred_eps'], noise, "eps", pred_type='EPSILON')

        terms = {}
        loss_eps = terms_eps["mse_eps"] + terms_eps.get("vb_eps", 0) + terms_eps.get("mae_eps", 0)
        terms["loss"] = loss_eps
        terms.update(terms_eps)
        return terms
